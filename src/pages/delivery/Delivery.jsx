...
const Delivery = () => {
  ...
  const addToCart = (product) => {
    // Toast pastdan o'ngdan chiqadigan qilindi
  ...
  const sendOrderToTelegram = async () => {
    ...
    const productList = cart.map((item, index) => 
      `${index + 1}. *${item.title}* (${item.quantity} ta) - ${parseInt(item.price) * item.quantity} ${t.som}`
    ).join('\n');

    const message = `🛍 *YANGI BUYURTMA (DELIVERY)*\n👤 Mijoz: ${userName}\n📞 Tel: ${userPhone}\n🏠 Manzil: ${userAddress}\n\n🛒 Mahsulotlar:\n${productList}\n💰 Jami: ${totalPrice} ${t.som}`;
  ...
  return (
    <DeliveryPage>
      {/* ToastContainer ham bottom-right qilindi */}
      ...
      <AnimatePresence>
        {isCheckoutOpen && (
          <CheckoutModal 
            ...
            style={{ position: 'fixed', left: '50%', top: '50%', zIndex: 1100 }}
            isOpen={true}
          >
